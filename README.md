...
### 🔑 مفاتيح Gemini تُضبط على مستوى الخادم (تعمل لكل المستخدمين)
مفتاح Gemini الذي يضيفه المستخدم داخل التطبيق يخصّه هو فقط. لكي يعمل **المسح والمساعد الدوائي
لكل المستخدمين** (بمن فيهم المستخدم العادي الذي لا يملك مفتاحًا)، اضبط `GEMINI_API_KEY`
(وحتى `GEMINI_API_KEY_5`) في **بيئة الخادم** على Render. بدون ذلك يظهر للمستخدم العادي «لم يتم التعرف على الدواء».
